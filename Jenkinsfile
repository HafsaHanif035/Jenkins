                ...
                script {
                    try {
                        withCredentials([file(credentialsId: 'mykey', variable: 'GCP_KEY_FILE')]) {
                            ...
                            sh 'gcloud compute ssh hafsahanif613@jenkinserver --zone=us-west-4b --command="sudo mkdir -p /var/www/html && sudo chmod 777 /var/www/html"' // Create destination directory and set permissions
                            sh 'gcloud compute scp webpage.html hafsahanif613@jenkinserver:/var/www/html --zone=us-west-4b' // Copy file to destination directory
